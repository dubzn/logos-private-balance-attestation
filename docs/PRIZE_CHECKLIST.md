...
| Reproducible demo script with `RISC0_DEV_MODE=0`. | partial | `scripts/demo-local-sequencer-e2e.sh` passed locally with wallet state + real `getProofForCommitment` + challenge-bound verify `status: ok`. `scripts/demo-local-gate-e2e.sh` passed locally from those public envelope/gate artifacts and persisted the nullifier in the LEZ gate account. Clean-room rerun/video still pending. |
