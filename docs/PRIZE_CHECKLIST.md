...
| Full docs and clean public repo. | in-progress | README + `docs/`, root `demo.sh`, root `balance-attestation-verifier.idl.json`, Basecamp `module.json`, CI, Basecamp MVP docs, clean-room checks, and `scripts/demo-clean-room.sh`; final testnet deployment docs, CU metrics, and video artifacts pending. |
