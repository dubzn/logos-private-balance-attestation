...
| E2E tests against standalone LEZ sequencer in CI. | done | `.github/workflows/ci.yml` starts a pinned official standalone LEZ checkout, creates an ephemeral wallet/private account, funds it, calls the real `getProofForCommitment` path, proves in `RISC0_DEV_MODE=1`, verifies, and uploads only sanitized reports. The live job passed in [GitHub Actions run 29247360268](https://github.com/dubzn/logos-private-balance-attestation/actions/runs/29247360268). |
| CI green on default branch. | done | [GitHub Actions run 29247360268](https://github.com/dubzn/logos-private-balance-attestation/actions/runs/29247360268) passed fmt, clippy, workspace tests, isolated deployable-program checks, and the live standalone-sequencer E2E on 2026-07-13. |
