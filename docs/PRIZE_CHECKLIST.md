...
| Off-chain path over Logos Messaging. | in-progress / Delivery wired | `attestation-verifier` + `attestation-messaging` + `examples/chat-gate`: envelope JSON is wrapped as a proof message, received/imported, verified locally, and admitted into a local token-gated group state. `apps/basecamp` now declares the real `delivery_module` dependency and wires create/start/subscribe/send/receive/verify for the same proof-message bytes. Final two-instance Delivery recording pending. |
...
| Requirement | Status | Artifact |
| --- | --- | --- |
| Document CU cost of on-chain operations. | planned / wall-clock timings documented | `docs/BENCHMARKS.md` records local and public testnet wall-clock timings and explicitly lists missing per-transaction CU measurements. |
...
| Deployable LEZ program | passed locally and on public testnet | `lez-verifier/program/` builds against `nssa_core` from the LEZ checkout; `cargo test --manifest-path lez-verifier/program/Cargo.toml --test program_journal_roundtrip` is 6/6 (register presenter + init + admit happy path + BA206 replay + BA203 wrong presenter + BA102 wrong inner image id; image id pinned). Public testnet deploy/admit succeeded on 2026-06-01, but receipt binding remains host-side. |
