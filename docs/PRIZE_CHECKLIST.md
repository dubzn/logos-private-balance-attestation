...
| Reproducible demo script with `RISC0_DEV_MODE=0`. | partial | `scripts/demo-local-sequencer-e2e.sh` passed locally with wallet state + real `getProofForCommitment` + challenge-bound verify `status: ok`. `scripts/demo-local-gate-e2e.sh` passed locally from those public envelope/gate artifacts and persisted the nullifier in the LEZ gate account. `scripts/demo-local-full-e2e.sh` now composes both phases; full clean-room rerun/video still pending. |
...
| Full local E2E harness | added / pending live run | `scripts/demo-local-full-e2e.sh` composes `demo-local-sequencer-e2e.sh` and `demo-local-gate-e2e.sh` into one operator command. Static checks pass; live full run still pending. |
